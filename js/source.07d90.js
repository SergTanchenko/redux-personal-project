...
const sortTasks = tasks => {
  const favoriteUnaccomplished = tasks.filter(task => task.get("favorite") && !task.get("completed"));
  const unaccomplished = tasks.filter(task => !task.get("favorite") && !task.get("completed"));
  const completed = tasks.filter(task => task.get("completed"));
  const sortedCompleted = completed.sort((t1, t2) => compareObjectsByProperty(t1, t2, "favorite"));
...
/***/ })
/******/ ]);
//# sourceMappingURL=source.07d90.js.map